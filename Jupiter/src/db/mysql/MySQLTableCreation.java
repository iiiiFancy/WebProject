	...
	public static void main(String[] args) {
		try {
			...
			try {
				...
				Class.forName("com.mysql.jdbc.Driver").getConstructor().newInstance(); // 把“com.mysql.jdbc.Driver”类加入DriverManager的list中
				// 此句强制JVM加载com.mysql.jdbc.Driver类，此时会执行其中静态代码块
				// .getConstructor()调用构造函数但不返回引用
				conn = DriverManager.getConnection(MySQLDBUtil.URL); // 遍历所有DriverManager中的driver寻找合适的一个建立连接
			...
			sql = "DROP TABLE IF EXISTS history"; // 要先删除声明了foreign key的表
			stmt.executeUpdate(sql);
			...
			// Step 4: test, insert a fake user 3229c1097c00d497a0fd282d586be050
